...
Object.defineProperties(CMake.prototype, {
    path: {
        get: function () {
            return this.options.cmakePath || process.env.CMAKE_PATH || "cmake";
        }
    },
    isAvailable: {
        get: function () {
...
CMake.isAvailable = function (options) {
    options = options || {};
    try {
        let cmakePath = options.cmakePath || process.env.CMAKE_PATH;
        if (cmakePath) {
            let stat = fs.lstatSync(cmakePath);
...
CMake.getGenerators = async function (options, log) {
    ...
    if (CMake.isAvailable(options)) {
        ...
        try {
            let stdout = await processHelpers.execFile([options.cmakePath || process.env.CMAKE_PATH || "cmake", "-E", "capabilities"]);
        ...
        let stdout = await processHelpers.execFile([options.cmakePath || process.env.CMAKE_PATH || "cmake", "--help"]);
